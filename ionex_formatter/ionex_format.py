        ...
        for label, format in self.HEADER_FORMATS.items():
            ...
            for token in format_tokens:
                ...
                if token[-1] == 'A' or token[0].isalpha():
                    ...
                    continue
                # raise ValueError('Unknown format token {}'.format(token)) - Закомментировал эту строку, 
                # так как невозможно её покрыть из-за того, что нужно менять переменную HEADER_FORMATS
